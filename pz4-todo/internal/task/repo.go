package task

import (
	"encoding/json"
	"errors"
	"log"
	"os"
	"sync"
	"time"
)

var ErrNotFound = errors.New("task not found")

type Repo struct {
	mu       sync.RWMutex
	seq      int64
	items    map[int64]*Task
	filePath string
}

func NewRepoWithFile(filePath string) (*Repo, error) {
	repo := &Repo{
		items:    make(map[int64]*Task),
		filePath: filePath,
	}

	// Загружаем данные из файла при создании
	if err := repo.loadFromFile(); err != nil {
		return nil, err
	}

	return repo, nil
}

func NewRepo() *Repo {
	return &Repo{items: make(map[int64]*Task)}
}

func (r *Repo) loadFromFile() error {
	if r.filePath == "" {
		return nil // нет файла - ничего не загружаем
	}

	// Проверяем существует ли файл
	if _, err := os.Stat(r.filePath); os.IsNotExist(err) {
		return nil // файла нет - начинаем с пустого репозитория
	}

	// Читаем файл
	data, err := os.ReadFile(r.filePath)
	if err != nil {
		return err
	}

	// Парсим JSON
	var tasks []*Task
	if err := json.Unmarshal(data, &tasks); err != nil {
		return err
	}

	// Загружаем задачи в репозиторий
	r.mu.Lock()
	defer r.mu.Unlock()

	r.items = make(map[int64]*Task)
	var maxID int64 = 0

	for _, task := range tasks {
		r.items[task.ID] = task
		if task.ID > maxID {
			maxID = task.ID
		}
	}

	r.seq = maxID // устанавливаем последовательность на максимальный ID

	return nil
}

func (r *Repo) saveToFile() error {
	if r.filePath == "" {
		return nil // нет файла - ничего не сохраняем
	}

	r.mu.RLock()
	tasks := make([]*Task, 0, len(r.items))
	for _, task := range r.items {
		tasks = append(tasks, task)
	}
	r.mu.RUnlock()

	// Форматируем JSON с отступами
	data, err := json.MarshalIndent(tasks, "", "  ")
	if err != nil {
		return err
	}

	// Сохраняем в файл
	return os.WriteFile(r.filePath, data, 0644)
}

func (r *Repo) List() []*Task {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]*Task, 0, len(r.items))
	for _, t := range r.items {
		out = append(out, t)
	}
	return out
}

func (r *Repo) Get(id int64) (*Task, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	t, ok := r.items[id]
	if !ok {
		return nil, ErrNotFound
	}
	return t, nil
}

func (r *Repo) Create(title string) *Task {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.seq++
	now := time.Now()
	t := &Task{ID: r.seq, Title: title, CreatedAt: now, UpdatedAt: now, Done: false}
	r.items[t.ID] = t

	go func() {
		if err := r.saveToFile(); err != nil {
			log.Printf("Failed to save tasks to file: %v", err)
		}
	}()

	return t
}

func (r *Repo) Update(id int64, title string, done bool) (*Task, error) {
	r.mu.Lock()
	defer r.mu.Unlock()
	t, ok := r.items[id]
	if !ok {
		return nil, ErrNotFound
	}
	t.Title = title
	t.Done = done
	t.UpdatedAt = time.Now()

	go func() {
		if err := r.saveToFile(); err != nil {
			log.Printf("Failed to save tasks to file: %v", err)
		}
	}()

	return t, nil
}

func (r *Repo) Delete(id int64) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if _, ok := r.items[id]; !ok {
		return ErrNotFound
	}
	delete(r.items, id)

	go func() {
		if err := r.saveToFile(); err != nil {
			log.Printf("Failed to save tasks to file: %v", err)
		}
	}()

	return nil
}

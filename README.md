...
## Транзакции
Транзакция гарантирует, что либо вставятся все задачи, либо ни одна — нет частичных результатов при ошибке.

## Настройки пула

**MaxOpenConns(10):** На локалке 10 соединений достаточно, экономим память.

**MaxIdleConns(5):** Держим 5 соединений в готовности, чтобы новые запросы не создавали новые подключения.

**ConnMaxLifetime(30min):** Соединение автоматически пересоздаётся через 30 минут, защита от зависаний.
